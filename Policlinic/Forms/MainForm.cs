    ...
    public partial class MainForm : Form
    {
        ...
        public void SearchCardGridRenew(string surname, int year, string address)
        {
            ...
            if ((!String.IsNullOrEmpty(surname)) || (year != 0) || (!String.IsNullOrEmpty(address)))
            {
                ...
                {
                    if ((String.IsNullOrEmpty(surname)) || (patient.Surname.ToLower() == surname.ToLower()))
                    {
                        if ((year == 0) || (patient.Birthdate.Year == year))
                        {
                            if ((String.IsNullOrEmpty(address)) || patient.Address.ToLower().Contains(address.ToLower()))
                            {
                                ...
                                searchCardGridView[3, rowIndex].Value = patient.Policy;
                                searchCardGridView[4, rowIndex].Value = string.Format("{0:d}", patient.Birthdate);
                                searchCardGridView[5, rowIndex].Value = patient.Address;
                                searchCardGridView[6, rowIndex].Value = patient.Shelving;
                                searchCardGridView[7, rowIndex].Value = patient.Shelf;
                                rowIndex++;
                            }
                        }
                    }
            ...
            else if ((Regex.IsMatch(textPolicy.Text, @"[0-9]")) && (textPolicy.Text.Length == 6))
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        foreach (var i in bl.GetCategoriesList())
                        {
                            comboReceptionCategory.Items.Add(i.Name.ToString());
                        }
                        string[] text = bl.PrintCoupon(id).Split(',');
                        listGetCouponMessage.Items.Clear();
